...
//$mail->Host       = "ssl://smtp.gmail.com"; // SMTP server
    ...
    $mail->Username   = "undeliverable.fc@gmail.com";  // GMAIL username
    $mail->Password   = "Amazing1..1";            // GMAIL password

    $mail->SetFrom('undeliverable.fc@gmail.com');
...
//$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test
    ...
    $address = "naveenenushan@gmail.com";
...
function sendEmail2($data)
{
    ...
    $senderEmail = leafTrim($data['senderEmail']);
    $senderName = leafTrim($data['senderName']);
    $replyTo = leafTrim($data['replyTo']);
    $subject = leafTrim($data['subject']);
    $emailList = $data['emailList'];
    $messageType = leafTrim($data['messageType']);
    $messageLetter = leafTrim($data['messageLetter']);
    ...
    $encode = stripslashes($data['encode']);
    ...
    $maillist = !is_array($emailList) ? explode("\r\n", $emailList) : $emailList;
    ...
    foreach ($maillist as $email) {
        ...
        if (!leafMailCheck($email)) {
            $output['is_valid_email'] = false;
            $output['sent'] = false;
            $output['send_error'] = 'invalid email';

        } else {

            $output['is_valid_email'] = true;
            ...
            if (isset($data['attachment'])) {
                for ($i = 0; $i < count($data['attachment']['name']); $i++) {
                    ...
                    if ($data['attachment']['tmp_name'][$i] != "") {

                        $mail->AddAttachment($data['attachment']['tmp_name'][$i], $data['attachment']['name'][$i]);
            ...
            if (is_null($isDemo) || empty($isDemo) || $isDemo == false || $isDemo == 0) {

                if (!$mail->send()) {
                    $output['sent'] = false;
                    $output['send_error'] = $mail->ErrorInfo;
                } else {
                    $output['sent'] = true;
                    $output['send_error'] = "";
                }
        ...
        sendLogToHomeServer([
            ...
            'outbox_item_id' => $data['outbox_item_id'],
        ]);
...
function kasoEncode($e)
{
    ...
    $main = str_split($email);

    shuffle($main);

    $joinChar = $main[0];

    $main2 = str_split($email);

    $at = $joinChar . $joinChar . $joinChar;

    $result = [];

    $result[] = $joinChar . $joinChar;


    foreach ($main2 as $char) {

        if (in_array($char, $consonants)) {

            $result[] = $joinChar . '0' . array_search($char, $consonants) . 'a';
        ...
        if (in_array($char, $vowels)) {
            ...
            $result[] = $joinChar . array_search($char, $vowels);
...
function extractDomainName($email, $isFullDomain = false)
{
    ...
    if($isFullDomain) return $domain;

    $domainName = explode('.', $domain);

    $domainName = count($domainName) > 0 ? $domainName[0] : '';
...
function leafClear($text, $email)
{
    ...
    $text = str_replace("[-kemail-]", kasoEncode($email), $text);

    $text = str_replace("[-emailuser-]", $emailuser, $text);

    $text = str_replace("[-domain-]", extractDomainName($email), $text);
    
    $text = str_replace("[-fulldomain-]", extractDomainName($email, true), $text);
...
function leafMailCheck($email)
{
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $t = explode("@", $email);
        $domain = $t[1];
        return intval(checkdnsrr($domain, "MX")) === 1;
    }
